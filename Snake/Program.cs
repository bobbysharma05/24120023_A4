...
try
{
	...
	while (!closeRequested)
	{
		...
		map[X, Y] = Tile.Snake;
		// Bug 4 Fix: Display score in real-time
		Console.SetCursorPosition(0, 0);
		Console.Write($"Score: {snake.Count - 1}    ");

		// Bug 3 Fix: Non-blocking input check
...
void GetDirection()
{
	// Bug 1 Fix: Prevent snake from reversing into itself
...
void PositionFood()
{
	// Bug 2 & 5 Fix: Ensure food spawns only in open spaces and handle no-space case
	...
	if (possibleCoordinates.Count == 0) return; // Bug 5: Prevent crash if no space
